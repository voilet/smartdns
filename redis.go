...
type Redis struct {
	...
	Fallthrough    bool             // New field for fallthrough
	Forward        *forward.Forward // 添加 Forward 插件
	IpdbPath       string           // 添加 IPDB 路径
}
...
func (redis *Redis) AXFR(z *Zone) (records []dns.RR) {
	...
	for key := range z.Locations {
		if key == "@" {
			location := redis.findLocation(z.Name, z, "axfr")
			record := redis.get(context.Background(), location, z)
			soa, _ = redis.SOA(z.Name, z, record)
		} else {
			...
			location := redis.findLocation(fqdnKey, z, "axfr")
			record := redis.get(context.Background(), location, z)
...
func (redis *Redis) hosts(name string, z *Zone) []dns.RR {
	...
	location := redis.findLocation(name, z, "host")
	if location == "" {
		return nil
	}
	record = redis.get(context.Background(), location, z)
...
func (redis *Redis) findLocation(query string, z *Zone, qtype string) string {
	var (
		ok                                 bool
	...
	query = strings.TrimSuffix(query, "."+z.Name) + ":" + qtype
	log.Debugf("findLocation查询: %s", query)
...
func (redis *Redis) get(ctx context.Context, key string, z *Zone) *Record {
	defer func() {
		if r := recover(); r != nil {
			err := r.(error)
			fmt.Println("查询异常:", err)
	...
	//log.Debugf("查询类型: %s", ctx.Value("qtype"))
	qtype := strings.ToLower(ctx.Value("qtype").(string))
	var (
		err error
		val string
	)
	...
	//从上下文获取客户端hash
	public := ctx.Value("public").(string)
	if public == "public" {
		// 公网ip优先查询智能CDN记录
		isp := ctx.Value("smartIsp").(string)
		province := ctx.Value("smartProvince").(string)
		keys := []string{
			fmt.Sprintf("%s:smart:%s:%s", label, isp, province),
			fmt.Sprintf("%s:smart:%s", label, isp),
			fmt.Sprintf("%s", label),
		}
		//if public == "public" {
		for _, v := range keys {
			log.Debugf("keys hget: %s %s", redis.keyPrefix+z.Name+redis.keySuffix, v)
			val, err = redis.redisHGet(redis.keyPrefix+z.Name+redis.keySuffix, v)
			if err == nil && val != "" {
				break
			}
		}
	} else {
		log.Debugf("local hget: %s %s", redis.keyPrefix+z.Name+redis.keySuffix, label)
		val, err = redis.redisHGet(redis.keyPrefix+z.Name+redis.keySuffix, label)
		if err != nil {
			log.Debugf("local hget error: %s", err)
		}
	}
	...
	if err := json.Unmarshal([]byte(val), r); err != nil {
		log.Debugf("parse error : %s", val)
		return nil
	}

	// Sort the records based on weight
	sort.SliceStable(r.A, func(i, j int) bool {
		return r.A[i].Weight > r.A[j].Weight
	})

	// Apply weight randomness to shuffle records based on weight
	r.A = shuffleAByWeight(r.A)
	return r
}

func (redis *Redis) redisHGet(key, field string) (string, error) {
	conn := redis.Pool.Get()
...
func (redis *Redis) load(zone string) *Zone {
	...
	defer conn.Close()
	log.Debugf("Loading zone %s", redis.keyPrefix+zone+redis.keySuffix)
